...
var runInstall = (exports.runInstall = function(dir, callback) {
  var install = cp.spawn('npm', ['install'], {cwd: dir, stdio: 'inherit'});
  install.on('close', function(code) {
    if (code !== 0) {
 ...
 */
var overwritePackageConfig = (exports.overwritePackageConfig = function(
  manifest,
  overrides,
  callback
) {
  ...
  async.waterfall(
    [
      ...
      function(done) {
        ...
        promzard(inputs, {name: path.basename(path.resolve(dest))}, done);
      },
